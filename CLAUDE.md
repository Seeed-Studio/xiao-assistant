...
Search is hand-rolled scoring (no index, no external search lib) — matches on exact field, tag, expanded synonym, and per-word levels with different weights, sorted by score. Adding new searchable content means adding YAML entries; no re-index step is needed.
